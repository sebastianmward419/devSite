const Skills = 
[
    {skill: 'React'    , icon: 'react.png'    ,   id: 0},
    {skill: 'Express'  , icon: 'express.png'  ,   id: 1},
    {skill: 'Node'     , icon: 'node.png'     ,   id: 2},
    {skill: 'AWS'      , icon: 'aws.png'      ,   id: 3},
    {skill: 'Git'      , icon: 'git.png'      ,   id: 4},
    {skill: 'Postgres' , icon: 'Postgres.png' ,   id: 5},
    {skill: 'Mongo'    , icon: 'mongo.png'    ,   id: 6},
    {skill: 'Docker'   , icon: 'docker.png'   ,   id: 7},
    {skill: 'Jest'     , icon: 'jest.png'     ,   id: 8},
    {skill: 'Webpack'  , icon:'webpack.png'   ,   id: 9},
    {skill: 'mySQL'    , icon:'mysql.png'     ,   id: 10}
]

export default Skills;